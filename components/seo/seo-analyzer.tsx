...
function calculateLiveSEO(title: string, content: string, excerpt?: string, tags: string[] = []) {
  ...
  const words = plainText ? plainText.split(/\s+/).filter(w => w.length > 0) : []
  ...
  if (titleLen === 0) {
    ...
    checks.push({
      label: "Title Length",
      passed: false,
      message: `Title is too short (${titleLen} chars). Target is 40-60.`
    })
  } else if (titleLen > 60) {
    checks.push({
      label: "Title Length",
      passed: false,
      message: `Title is too long (${titleLen} chars). Target is 40-60.`
    })
  } else {
    checks.push({
      label: "Title Length",
      passed: true,
      message: "Ideal title length (40-60 characters)."
    })
  }
  ...
  if (wordCount === 0) {
    ...
    checks.push({
      label: "Content Length",
      passed: false,
      message: `Content is too short (${wordCount} words). Write at least 300 words.`
    })
  } else {
    checks.push({
      label: "Content Length",
  ...
  // Rule 3: Keywords Matching
  if (!tags || tags.length === 0) {
    checks.push({
      label: "SEO Tags",
      passed: false,
      message: "No tags added. Include 1-2 keywords/tags."
    })
  } else {
    ...
    const matchingTags = tags.filter(tag => {
      ...
      return t && (titleLower.includes(t) || contentLower.includes(t))
    })

    if (matchingTags.length === 0) {
      checks.push({
        label: "Tag/Keyword Matches",
        passed: false,
        message: "Tags do not match content or title."
      })
    } else {
      checks.push({
        label: "Tag/Keyword Matches",
        passed: true,
        message: `Found matches for tags: ${matchingTags.join(", ")}`
      })
    }
  }

  // Rule 4: Structure/Headings
  const hasHeadings = content ? /<h[2-6]/i.test(content) || /(^|\n)#{2,6}\s+/m.test(content) : false
  checks.push({
    label: "Content Headings",
    passed: hasHeadings,
    message: hasHeadings ? "Subheadings used." : "Add H2-H6 subheadings to structure your post."
  })

  // Rule 5: Alt Attributes
  const imgTags = content ? content.match(/<img[^>]+>/gi) || [] : []
  if (imgTags.length > 0) {
    const missingAlt = imgTags.some(img => !/alt\s*=\s*['"]\s*[^'"]+\s*['"]/i.test(img))
    checks.push({
      label: "Image Alt Tags",
      passed: !missingAlt,
      message: missingAlt ? "Images are missing alt descriptions." : "All images have alt tags."
    })
  } else {
    checks.push({
      label: "Images Alt Tags",
      passed: true,
      message: "No images to verify."
    })
  }

  // Rule 6: Link presence
  const hasLinks = content ? /<a[^>]+href/i.test(content) || /\[([^\]]+)\]\(([^)]+)\)/i.test(content) : false
  checks.push({
    label: "Content Links",
    passed: hasLinks,
    message: hasLinks ? "Links present." : "Add internal/external links to build authority."
  })
