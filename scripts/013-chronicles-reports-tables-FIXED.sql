-- WHISPR CHRONICLES - REPORTS & GENERATION TABLES (FIXED)
-- Comprehensive reporting system with scheduled reports and export capabilities
-- Fixed to remove references to auth.users (use profiles table instead)
-- Run this AFTER running 012-chronicles-settings-tables-FIXED.sql
...
CREATE TABLE IF NOT EXISTS chronicles_generated_reports (
  ...
  -- Generation Info
  generated_by_user_id UUID, -- No foreign key to auth table
...
CREATE TABLE IF NOT EXISTS chronicles_report_exports (
  ...
  user_id UUID NOT NULL, -- No foreign key to auth table
