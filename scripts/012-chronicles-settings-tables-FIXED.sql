-- WHISPR CHRONICLES - SETTINGS & CONFIGURATION TABLES (FIXED)
-- Comprehensive settings management and configuration storage
-- Fixed to remove ON CONFLICT references on non-unique constraint operations
-- Run this AFTER running 011-chronicles-analytics-tables-FIXED.sql
...
SELECT TRUE, TRUE, 10, 10, FALSE, FALSE
WHERE NOT EXISTS (SELECT 1 FROM chronicles_system_settings);
...
-- Insert default leaderboard settings (only if table is empty)
...
SELECT 10, 2, 5, 1, TRUE, TRUE, TRUE
WHERE NOT EXISTS (SELECT 1 FROM chronicles_leaderboard_settings);

-- Insert default gamification settings (only if table is empty)
...
SELECT 10, 1, 2, 5, 10, TRUE
WHERE NOT EXISTS (SELECT 1 FROM chronicles_gamification_settings);

-- Insert default monetization settings (only if table is empty)
...
SELECT 70.00, 100.00, TRUE, TRUE, 'weekly'
WHERE NOT EXISTS (SELECT 1 FROM chronicles_monetization_settings);

-- Insert default theme settings (only if table is empty)
INSERT INTO chronicles_theme_settings (
  platform_name, primary_color, secondary_color, enable_dark_mode
) 
SELECT 'Chronicles', '#6366f1', '#ec4899', TRUE
WHERE NOT EXISTS (SELECT 1 FROM chronicles_theme_settings);

-- Insert default notification settings (only if table is empty)
...
SELECT TRUE, TRUE, TRUE, TRUE, TRUE, 15, FALSE
WHERE NOT EXISTS (SELECT 1 FROM chronicles_notification_settings);
...
SELECT TRUE, TRUE, 'daily', TRUE, TRUE
WHERE NOT EXISTS (SELECT 1 FROM chronicles_email_settings);

-- Insert default rate limiting settings (only if table is empty)
...
SELECT 1000, 60, 10, TRUE
WHERE NOT EXISTS (SELECT 1 FROM chronicles_rate_limiting_settings);
